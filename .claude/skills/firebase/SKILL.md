...
etc.) in any Firestore collection here — the `travelers` collection
was deliberately trimmed down to name-only for exactly this reason. If
a task asks you to add a sensitive field back, push back and confirm
that's really intended given everything here is public.

## Firestore collections currently in use

All declared with `allow read, write: if true;` in `firestore.rules`:
...
- `travelers` — traveler **names only** (nothing sensitive), used in `src/pages/Settings.jsx` via `useFirestoreCollection('travelers')`

## Adding a new Firestore-backed collection

Follow the existing pattern rather than inventing a new one:

1. **In `firestore.rules`**, add a new `match /yourCollection/{id} { allow read, write: if true; }` block (mirror the `bookings`/`travelers` blocks). Don't forget rules only take effect once deployed (see below) — editing the file alone does nothing in production.
